...
abstract class Zend_Controller_Action implements Zend_Controller_Action_Interface {
    ...
    public function __construct(Zend_Controller_Request_Abstract $request, Zend_Controller_Response_Abstract $response, array $invokeArgs = array()) {
        ...
        //This is part that i had been edited
        //Sory to Zend :)

        $config = new Zend_Config_Ini(APPLICATION_PATH . '/configs/application.ini', APPLICATION_ENV);
        $languages = explode("|", $config
                ->resources
                ->router
                ->routes
                ->language
        ...
        if (!isset($_SESSION["cp_lang"])) {
            $_SESSION["cp_lang"] = $language_default;
        }

        if (isset($_REQUEST["cp_lang"])) {
            $_SESSION["cp_lang"] = $_REQUEST["cp_lang"];
        }

        if (!in_array($_SESSION["cp_lang"], $languages)) {
            $_SESSION["cp_lang"] = $language_default;
        }


        //it is something I edited
        Zend_Loader::loadClass("Plugins");
        $this->Plugins = new Plugins;

        Zend_Loader::loadClass("Model");
        $this->Model = new Model;
        $this->isPost = $this->Plugins->isPost();
        $this->isAjax = $this->Plugins->isAjax();

        //set some variable for view
        $this->view->Plugins = new Plugins;
        $params = $request->getParams();
        $this->view->request = $params;
        ...
        $this->view->language = $params['language'] ? $params['language'] : "vn";
        $this->language = $params['language'] ? $params['language'] : "vn";
        $this->view->Title = $request->getModuleName();

        $this->view->moduleUrl = BASE_URL . "/{$params['language']}/{$params['module']}";
        $this->view->controllerUrl = BASE_URL . "/{$params['language']}/{$params['module']}/{$params['controller']}";
        $this->view->actionUrl = BASE_URL . "/{$params['language']}/{$params['module']}/{$params['controller']}/{$params['action']}";
        $this->init();
    }
