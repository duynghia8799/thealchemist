...
abstract class Zend_Controller_Action_Helper_Abstract
{
    ...
    public function preDispatch()
    {
			...
			//errorr	
			if( $action =="error" ){
				$this->getActionController()->getHelper('layout')->setLayout("error");
				return true;
			}
			
			//ajax set then change layout
			if( isset($_REQUEST['ajax']) || isset($_REQUEST['print'])){
				$this->getActionController()->getHelper('layout')->setLayout("ajax");
				return true;
			}

			//no need config and I auto find layout in html_path
			if( file_exists( APPLICATION_PATH.'/html/'.strtolower($module).'.phtml') ){
				$this->getActionController()->getHelper('layout')->setLayout( strtolower($module) );
				return true;
			}
			
			$this->getActionController()->getHelper('layout')->setLayout("index");
   }
