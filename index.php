<?php
session_start();
$root = str_ireplace('\\', '/', dirname($_SERVER['PHP_SELF']));
$root = preg_replace('/^\/$/is', '', $root);
...
defined('ROOT') || define('ROOT', dirname(APPLICATION_PATH));
defined('FILE_PATH') || define('FILE_PATH', ROOT . '/files');
...
function decimal($number, $dec_point = '2') {
    if ($number === null || $number === '' || !is_numeric($number))
        return '';

    $number = number_format($number + 0, $dec_point, '.', ',');
    $number = preg_replace("/[0]+$/is", '', (string) $number);
...
function path($file = null) {
    ...
    $file = preg_replace("#^files(\$|\/|\\\)#i", "$1", $file);
